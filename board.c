...
bool parseFEN(char *fen, S_BOARD *pos) {
  int rank = RANK_8, file = FILE_A;
  unsigned count = 0, piece = EMPTY;
  ...
  if (*fen != '-')
    pos->enPas = FR2SQ(fen[0] - 'a', fen[1] - '1');
